...
#define CHU11_SEVER_IP		("192.168.1.31")     //gai
// #define CHU10_SEVER_IP		("192.168.1.26") 



void CHU11Recv_Server_Deal(uint8_t* recv_buf,uint16_t RecvData_dlc,uint8_t CHU_num)
{
...
//			printf("get 0x83! %d %d %d %d\n",CHU_ErrCode_buf0[CHU_num],CHU_ErrCode_buf1[CHU_num],CHU_ErrCode_buf2[CHU_num],CHU_ErrCode_buf3[CHU_num]);
...
//			printf("get 0x89!=%d\n",recv_buf[13]);
		...
		case 0x8A://获取单体电压信息指令反馈
			...
			for(i=0;i<112;i++)
			{
				...
				// printf("get 0x8A! %d",BMS_CellVol_buf[10][0]);
			}
		break;
...
void CHU11_Rec_Deal(uint8_t CHU_num,uint8_t* recv_buf,uint16_t len)
{
	uint16_t i=0;
    ...
    do
    {
		if(recv_buf[current_head+0]==0xfe&&recv_buf[current_head+1]==0x01)
        {
			...
			// if(recv_buf[current_head+7]==0x8a){
			// 	printf("lenlen83333333: %d %d\n",len,RecvData_dlc);
			// }
            if(my_calchkval == rec_calchkval)
            {
				for(i=0;i<RecvData_dlc+13;i++)
				  buf[i] = recv_buf[current_head+i];
				CHU11Recv_Server_Deal(buf,RecvData_dlc,CHU_num);
                ss++;
...
void pthread_CHU11(void)
{
	struct sockaddr_in servaddr2;
	uint8_t CHU_Recv_buf[500];
	uint8_t CHU_index = 10;
	int16_t read_size;
...
//	printf("CHU%d_clientsockfd = %d \n",CHU_index,CHU_clientsockfd[CHU_index]);
    ...
    servaddr2.sin_addr.s_addr = inet_addr(CHU11_SEVER_IP);  //服务器ip
...
//			printf("connect to CHU%d failed.\n",CHU_index);
	...
	{
		usleep(10*1000);//10ms
		if(CHUSend_Discon_cou[CHU_index]>=5)
		{			
			mid_charger.EMS_CHU_LinkState[CHU_index] = 2;
			int8_t Send_flg = -1;
			...
			while(Send_flg<0)
			{
				...
				fprintf(stderr, "connect chu11111 function failed.\n");
				mid_charger.EMS_CHU_LinkState[CHU_index] = 2;
				//sleep(10);
		...
		// if((recv(clientsockfd, Qt_Recv_buf, BUFFER_SIZE,MSG_DONTWAIT|MSG_NOSIGNAL))<0){
			...
			CHU11_Rec_Deal(CHU_index,CHU_Recv_buf,Read_Size1);	
			CHUSend_Discon_cou[CHU_index] = 0;
			mid_charger.EMS_CHU_LinkState[CHU_index] = 1;
		}
	}	
