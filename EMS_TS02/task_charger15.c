...
#define CHU15_SEVER_IP		("192.168.1.35")     //gai
// #define CHU10_SEVER_IP		("192.168.1.26") 



void CHU15Recv_Server_Deal(uint8_t* recv_buf,uint16_t RecvData_dlc,uint8_t CHU_num)
{
...
void CHU15_Rec_Deal(uint8_t CHU_num,uint8_t* recv_buf,uint16_t len)
{
	uint16_t i=0;
    ...
    do
    {
		if(recv_buf[current_head+0]==0xfe&&recv_buf[current_head+1]==0x01)
        {
            ...
            if(my_calchkval == rec_calchkval)
            {
				...
				CHU15Recv_Server_Deal(buf,RecvData_dlc,CHU_num);
                ss++;
...
void pthread_CHU15(void)
{
//	printf("pthread_CHU15 tid %d\n",gettid());
	            	// char * test;
            	// strcpy(test,0);
	struct sockaddr_in servaddr2;
	uint8_t CHU_Recv_buf[500];
	uint8_t CHU_index = 14;
	int16_t read_size;
    ...
    servaddr2.sin_addr.s_addr = inet_addr(CHU15_SEVER_IP);  //服务器ip
...
//			printf("connect to CHU%d failed.\n",CHU_index);
	...
	while (1)
	{
		...
		// if((recv(clientsockfd, Qt_Recv_buf, BUFFER_SIZE,MSG_DONTWAIT|MSG_NOSIGNAL))<0){
			...
			// printf("Read_Size1 %d\n",Read_Size1);
			CHU15_Rec_Deal(CHU_index,CHU_Recv_buf,read_size);	
